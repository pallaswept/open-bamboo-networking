...
std::string setup_h2s_home(const std::string& dev_id) {
...

    {
        ...
        if (stat(src_plugins.c_str(), &pst) == 0) {
            if (symlink(src_plugins.c_str(), (cfg + "/plugins").c_str()) != 0)
                LOG_W("symlink plugins/ failed: %s", strerror(errno));
        } else {
            ...
            if (!g_plugin_path_for_home.empty()) {
                ...
                if (symlink(pp.c_str(), (cfg + "/plugins/" + bname).c_str()) != 0)
                    LOG_W("symlink plugin into plugins/ failed: %s", strerror(errno));
        ...
        if (stat(src_system.c_str(), &pst) == 0) {
            if (symlink(src_system.c_str(), (cfg + "/system").c_str()) != 0)
                LOG_W("symlink system/ failed: %s", strerror(errno));
        } else {
...
pid_t launch_daemon(const std::string& daemon_exe,
    ...
    if (intermediate != 0) {
        ...
        pid_t daemon_pid = -1;
        if (read(pipefds[0], &daemon_pid, sizeof(daemon_pid)) != (ssize_t)sizeof(daemon_pid)) {
            LOG_E("failed to read daemon pid from pipe: %s", strerror(errno));
            daemon_pid = -1;
        }
        close(pipefds[0]);
    ...
    if (daemon_pid != 0) {
        close(notif_pipe[1]);
        if (write(pipefds[1], &daemon_pid, sizeof(daemon_pid)) != (ssize_t)sizeof(daemon_pid))
            _exit(3);
        close(pipefds[1]);
        _exit(0);
    }
        ...
        if (!use_supervisor) {
            ...
            if (d) {
                ...
                while ((ent = readdir(d)) != nullptr) {
                    ...
                    int ppid_val = 0;
                    char name[256] = {};
                    int matched = fscanf(sf, "%*d (%255[^)]) %*c %d", name, &ppid_val);
                    fclose(sf);
                    if (matched != 2 || ppid_val != (int)daemon_pid) continue;
